...
def pc_calculate(seed, p, sizex, sizey, scale):
    ...
    g = UndirectedGraph.MakeRectangularSitePercolation(
        sizex, sizey, scale, p, seed, False
    )
    g.Fill()
...
def s_sizes(g, clusters):
    d = (
        {}
    )  # an empty dict that is going to contain keys:sizes and values: the index in clusters list
    i = 0
    for cl in clusters:
        lenght = len(cl)
...
def verify(seed, p, sizex, sizey, scale):
    ...
    g.Fill()
    clusters = g.FindAllClusters()
    NetGraphics.DrawSquareNetworkSites(
        L,
        H,
        p,
        nodelists=clusters,
...
def connect(sizex, sizey, scale, pc, path):
    ...
    ax1.plot(
        probs, eps_0, marker="D", linestyle=" ", markersize=6, markerfacecolor="None"
    )
    ax2.errorbar(
        p_r,
        eps_0,
    ...
    fig.savefig(
        path + "\\" "connected_averaged_over_seeds_3.png"
    )  # change this for each case
    d = {
        "p": probs,
        "epsilon": eps_0,
        "eps_dev": dev,
...
def P_inf(sizex, sizey, scale):
    ...
    df.to_csv(
        path + r"\videos y fotos medidas\Results\30x30NFS\p_inf\data_30x30NFS_Pinf.csv",
        index=0,
    )
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(16, 8))
    ax1.errorbar(
    ...
    fig.savefig(
        path + r"\videos y fotos medidas\Results\30x30NFS\p_inf\pinf_30x30NFS.png"
    )
...
def ns_vs_s(sizex, sizey, scale, path):
    ...
    for p in probs:
        ...
        for seed in seeds:
            ...
            plt.legend()
            plt.savefig(
                path + r"\log(ns)Vslog(s)" + "\\" "p=%2.2f" % (p) + ".png"
            )  # change this for each case
...
def sVSp(sizex, sizey, scale, path):
    ...
    d = {
        ...
        "dev": dev,
        "p_real": p_real,
        "p_real_dev": p_real_dev,
    }
    df = pd.DataFrame.from_dict(d)
    df.to_csv(path + "\\" "10x10FS_SvsP.csv", index=False)
    fig.savefig(
        path + "\\" "10X10FS_points= " + str(len(probs)) + ".png"
    )  # change this for each case
...
def snsVSp(sizex, sizey, scale):
    ...
    ax_params(ax2, xlabel="$p_{real}$", ylabel=None)
    fig.suptitle(
        r"cluster size distribution $\sum sn_s$ vs p, points= " + str(len(probs))
    )
    fig.savefig(
        path + r"\videos y fotos medidas\Results\30x30NFC\sumSnsp\snsp_30x30NFS.png"
    )  # change this for each case
    data.to_csv(
        path
        + r"\videos y fotos medidas\Results\30x30NFC\sumSnsp\data_sns_30x30NFS.csv",
        index=False,
    )
