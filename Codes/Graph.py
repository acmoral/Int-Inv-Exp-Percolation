...
class UndirectedGraph:
    ...
    def FindClusterFromNode(self, node, visited=None):
        if visited is None:
            visited = dict(
                [(n, False) for n in self.GetNodes()]
            )  # Creates a dictionary of each node as key and with all values set to False
        ...
        while (
            len(currentShell) > 0
        ):  # if i still have somewhere to flow to, say neighbors, continue
            nextShell = []
            for node in currentShell:
                if not visited[node]:  # if it hasnt been checked
                    nextShell.extend(
                        self.GetNeighbors(node)
                    )  # get the neighbors of neighbor at the end of list
    ...
    def FindAllClusters(self):
        ...
        for node in self.GetNodes():
            if not visited[node]:
                cluster = self.FindClusterFromNode(
                    node, visited
                )  # this visited dict is modified in FindClustedfromNode
    ...
    def drawhist(self, rect, p, bars, sizex, sizey):
        ...
        if bars == True:
            if (
                p > 0.55
            ):  # I changed this for the circular case because it gets messy at this point
        ...
        if rect:
            ...
            Areah = Area / 2
            ocupated = (
                nodesin * circle + (self.counter * Area) + (self.halfcounter * Areah)
            )
    ...
    def PlotCircular(p, seed, sizex, sizey, scale, bars=True):
        ...
        L = g.L
        H = g.H
        NetGraphics.DrawCircularNetworkSites(
            borderup,
            borderdown,
            squares,
            L,
    ...
    def PlotRectReal(p, seed, sizex, sizey, scale, bars=True):
        ...
        NetGraphics.DrawSquareNetworkSites(
            L, H, p, sizex, sizey, cl, scale, seed, change=False
        )
        NetGraphics.DrawSquareNetworkSites(
            L, H, p, sizex, sizey, cl, scale, seed, change=True
        )
    ...
    def preal_plt(sizex, sizey, scale):
        ...
        df.to_csv(
            r"C:\Users\Carolina\OneDrive\Escritorio\Int inv Exp\videos y fotos medidas\Results\30X5FS\pvsp_real.csv",
            index=False,
        )
        ...
        fig.savefig(
            path + r"\videos y fotos medidas\Results\30X5FS" + "\\" "pvsp_real.png"
