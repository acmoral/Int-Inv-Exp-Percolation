...
for i in range(8):
    ...
    X0, Y0, weight0, height0 = p0.returns(
        os.path.join(path, "p=" + names[i] + ".csv"), 1, stop0, probs[i]
    )
    ax1.errorbar(
        weight0,
        height0,
        ...
        ms=8,
        c="black",
        fillstyle=next(cyful),
        markeredgewidth=2,
    )
    ax1.text(
        weight0[len(weight0) - 1] - 0.65,
        height0[len(height0) - 1] + 0.003,
        str(probs[i]),
        fontsize=20,
    )
...
showcase.plot_youngs(
    df, [0, 0.05418, 0.10285, 0.15404, 0.20271, 0.24770, 0.29316, 0.34435]
)
showcase.plot_youngs_log_log(
    df,
    [0, 0.05418, 0.10285, 0.15404, 0.20271, 0.24770, 0.29316, 0.34435],
    pc_real,
    pc_ideal,
)
