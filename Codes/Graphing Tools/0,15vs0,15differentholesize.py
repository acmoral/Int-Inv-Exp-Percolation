...
ax1.errorbar(
    ...
    ls=" ",
    ms=6,
    label=r"Ocupación=2279,Area=2279$mm^2$",
    color="black",
)
...
# ---------------------p=0,05A=3mmH=1mm----------------------------------------
df = pd.read_csv(
    r"C:\Users\Carolina\OneDrive\Escritorio\Int inv Exp\videos y fotos medidas\excel\p=0,15H=2mmA=3mm.csv"
)
y = df["y"]
height = -y[::4] * 100
height = height[:39]
...
ax1.errorbar(
    ...
    ls=" ",
    ms=9,
    label=r"Ocupación=571,Area=2284$mm^2$",
    color="seagreen",
)
