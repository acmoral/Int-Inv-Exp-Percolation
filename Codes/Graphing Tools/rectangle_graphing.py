...
class showcase:
     ...
     def plot_youngs(data,probs):
       ...
       ax1.errorbar(probs,data['E']/(10**9),xerr=data['p_err'],yerr=data['Eerror']/(10**9),marker='D',ls='',ms=7,markerfacecolor="None", markeredgewidth=2,color='black')
       ax1.set_xlabel(r'Probabilidad',fontsize=30)
       ax1.set_ylabel(r'Modulo de Young $[GPa]$',fontsize=30) 
     ...
     def plot_youngs_log_log(data,probs,pc_real,pc_ideal):
       ...
       ax1.set_xlabel(r'$Log \enspace|p_c-p|$',fontsize=30)
       ax1.set_ylabel(r'$Log \enspace|E|$',fontsize=30) 
       ax1.tick_params(axis='both', labelsize=20)
       ax1.yaxis.offsetText.set_fontsize(20)
       ax1.xaxis.set_major_formatter(ticker.StrMethodFormatter("{x:.2f}"))
